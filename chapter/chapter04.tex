...
由于偏航角$\psi$对DFUAV的飞行轨迹不产生影响，所以在位置控制时可以人为自主设定期望的偏航角。一般情况下设定期望的偏航角为实际运动方向与地理北极的夹角。例如在轨迹跟踪过程中，假设当前时刻的参考位置为$\boldsymbol{P}^e_d=[x^e_d\quad y^e_d\quad z^e_d]^T$，上一时刻的参考位置为$\boldsymbol{P}^e_{d-1}=[x^e_{d-1}\quad y^e_{d-1}\quad z^e_{d-1}]^T$，则期望的偏航角$\psi_d$可以设置为：
\begin{equation}
...
实验过程中期望的偏航角始终指向正北方向，即($\psi_d=0$)，初始位置为$\boldsymbol{P}^e_{start}=[0 \quad 0 \quad -10]^Tm$，期望位置指令设置为$\boldsymbol{P}^e_{d}=[0 \quad 0 \quad -10]^Tm$，即保持定点。仿真时长为10s，仿真步长为0.01s，扰动在第1s时刻添加。图\ref{INDI位置控制定点悬停抗风实验仿真结果}展示了分别使用INDI位置控制器和PID位置控制器的实验过程中DFUAV的位置随时间变化的响应曲线。
\begin{figure}[htbp]
	\centering
	\begin{minipage}[c]{1\textwidth}
        \centering
        \includegraphics[scale=1]{Fig/INDI位置控制定点悬停抗风实验仿真结果.pdf}
        \caption{\label{INDI位置控制定点悬停抗风实验仿真结果}INDI位置控制定点悬停抗风实验仿真结果}
        \end{minipage}
\end{figure}
