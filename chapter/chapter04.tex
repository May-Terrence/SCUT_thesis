...
为了检验本章提出的基于INDI的位置控制器设计的有效性，本节进行了一系列仿真实验与实际飞行试验。仿真试验包括验证INDI位置控制器在外部环境风速扰动下的性能，以及INDI位置控制器的轨迹跟踪性能，仿真实验均使用PID位置控制器作为比较。在串级PID位置控制方法中，位置环和速度环均采用了PI控制器。实际飞行实验中使用INDI位置控制器跟踪方形轨迹和圆形轨迹以验证控制器的轨迹跟踪能力。实验过程中使用的控制器参数如表\ref{outloop_Control_parameters}所示。仿真实验平台和实际飞行实验平台均与第三章的姿态控制实验相同，为了进行可变控制，在验证位置控制器性能时，内环的姿态控制器均采用第三章设计的基于INDI和优先级控制分配的姿态控制器。
\begin{table}
...
本小节通过实际飞行实验验证所提出的INDI位置控制器的轨迹跟踪性能。实验中选取了方形轨迹和圆形轨迹作为参考轨迹，以验证试验样机的点对点轨迹跟踪性能和圆弧轨迹跟踪性能。

\subsubsection{点对点轨迹飞行实验}

点对点轨迹飞行实验的参考轨迹为40m $\times$ 40m的方形，期望高度保持为-5m。初始位置为$\boldsymbol{P}^e_{start}=[0 \quad 0 \quad -5]^T$m，初始速度为$\boldsymbol{V}^e_{start}=[0 \quad 0 \quad 0]^T$m/s，最大平飞速度设置为2m/s。期望的偏航角始终指向期望轨迹的方向，由式\eqref{4_25}计算得到。实验过程中，试验样机在地面坐标系下的位置跟踪曲线、速度跟踪曲线、姿态跟踪曲线和二维平面轨迹分别如图\ref{点对点轨迹飞行实验位置跟踪结果}到图\ref{点对点轨迹飞行实验二维平面轨迹结果}所示。

根据实验结果可知，试验样机在整个轨迹跟踪过程中均能够保持在期望轨迹附近，实验过程中X方向的最大位置误差为0.47m，Y方向的最大位置误差为0.34m，Z方向的最大位置误差为0.085m，这是由于X/Y方向上的位置控制与期望的姿态角$\varphi_d$和$\theta_d$的跟踪精度有关，而高度通道的控制与姿态角无关，控制较为简单，因此高度误差相对较小。
\begin{figure}[htbp]
	\centering
	\begin{minipage}[c]{1\textwidth}
        \centering
...
对于速度响应曲线，仅在每段加减速过程中有少许超调量，X方向和Y方向的最大速度误差均为0.7m/s，但是在加减速过程结束后，速度响应曲线均能够较好地稳定在期望速度上。由于飞行过程中期望偏航角$\psi_d$始终沿着飞行方向，因此DFUAV的俯仰角会经历较大变化，根据姿态跟踪曲线可以看出，俯仰角的跟踪效果较好，最大误差为$3.89^{\circ}$,无明显超调。滚转角和俯仰角的跟踪误差同样较小，位于合理范围内。
