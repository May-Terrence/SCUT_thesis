...
根据上述分析，INDI输入的计算依赖于机体的角加速度值$\dot {\boldsymbol{\omega}}_0^b$，在现有条件下，该值可以通过机体陀螺仪测量的角速度进行一阶差分得到：
\begin{equation}
    ...
    \label{3-49}
\end{equation}
其中$\Delta t$是陀螺仪传感器的采样时间。因此INDI也被称为基于传感器的方法。

为了提高姿态控制器的响应速度和控制精度，$\Delta t$应该在条件允许的情况下尽可能小。但是传感器测量得到的角速度会受到机体震动和外部扰动等因素的影响，难免会有噪声，越小的采样时间更会放大这种噪声的影响。因此在实际应用中，需要对角速度进行滤波处理。但是滤波处理就会在角加速度的计算中引入延迟，由于INDI是将角速度动力学方程在工作点处进行一阶泰勒展开，所以需要保证展开式\eqref{3-41}中所有下标为0的项都位于同一工作点。一种方法是对角加速度进行预测，但是这种方法需要对角加速度进行额外的建模，并且角速度的干扰也无法预测。或者采用多个加速度计来直接测量角加速度，但是这种方式会导致机载航电系统设计过于复杂，并且加速度计受机体震动影响也较大。因此，本文针对所有下标为0的项进行同步滤波处理，除了角加速度项外，还包括虚拟控制输入项和风扇转速输入项。文献\parencite{smeurAdaptiveIncrementalNonlinear2015,smeurCascadedIncrementalNonlinear2018b}通过四旋翼飞行试验表明这种处理方式的可行性。本文实验使用的滤波器是二阶Butterworth低通滤波器，滤波器截止频率的具体数值应该根据飞行过程中信号的频率成分、带宽和噪声水平来综合确定。对于本文使用的DFUAV，滤波器截止频率为30Hz。为加以区分，所有下标为0的项经过滤波器滤波后，使用下标$(.)_f$表示。
...
根据式\eqref{eq_35}、\eqref{eq_36}和式\eqref{3-14}，可用于理论计算的控制舵面力矩可表示为：
\begin{gather}
