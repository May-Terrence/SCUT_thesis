...
本章安排如下：将首先对DFUAV的姿态模型进行简化处理，方便下文姿态控制算法的设计。然后简要概述INDI和控制分配的理论以及二者的结合，继而给出了基于INDI和优先级控制分配的DFUAV的姿态控制方案设计。为验证方案有效性，最后进行了仿真实验和飞行实验验证。

\section{姿态模型简化}

本节将从两个方面简化姿态模型：一是简化非主要气动力矩的表示，区分可控力矩与不可控力矩。二是使用虚拟控制输入来表示姿态子系统的控制输入和控制分配算法的期望力矩。
...
\begin{equation}
    ...
    \label{eq_3-7}
\end{equation}
控制舵面合力矩$\boldsymbol{M}_{vane}^b$作为可控力矩由下文设计的姿态控制算法来计算期望力矩（即期望的控制舵面偏转角度），陀螺力矩$\boldsymbol{M}_{gyro}^b$同样作为可控力矩会对DFUAV的姿态造成负面影响，将由下文设计的陀螺力矩补偿算法来加以补偿。而$\boldsymbol{M}_{a}^b$作为不可控力矩将通过姿态控制算法在在线补偿。
