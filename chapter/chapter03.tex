...
添加干扰情况下的实验对比结果如图\ref{INDI对比PID仿真有扰动}所示，扰动在方波输入时同步添加。在横滚通道上，INDI控制器在添加扰动后仍然能够保持较好的跟踪性能，这是由于INDI在补偿未建模动态的同时也纳入了对外在扰动的补偿，并且在每一个控制周期内都实时补偿而不会积累误差。但是PID控制器则在面对扰动的情况下表现出了明显的滞后与超调量，且当INDI控制达到稳态时，PID控制器仍存在明显的偏差。在偏航通道上，INDI对于扰动引起的偏航力矩的抑制能力也明显优于PID。
\begin{figure}[htbp]
	\centering
	\begin{minipage}[c]{1\textwidth}
        \centering
...
% 姿态通道的实验结果如图\ref{优先级伪逆姿态仿真}所示。在横滚通道上，优先级控制分配算法在控制舵面饱和时能够更好地保持期望姿态，而伪逆方法则会出现控制舵面超出约束的情况。在俯仰通道上，优先级控制分配算法在控制舵面饱和时能够更好地保持期望姿态，而伪逆方法则会出现控制舵面超出约束的情况。在偏航通道上，优先级控制分配算法在控制舵面饱和时能够更好地保持期望姿态，而伪逆方法则会出现控制
% \begin{figure}[htbp]
% 	\centering
% 	\begin{minipage}[c]{1\textwidth}
%         \centering
