...
为方便检验本文的研究成果，自主搭建了一架DFUAV试验样机。本章首先简要介绍试验样机的系统组成和样机上搭载的航电系统，接下来重点是对DFUAV进行系统建模，这将是后续进行飞行控制算法设计的基础。物体的相对运动离不开其所处的参考坐标系，因此在建模之前将先介绍本文所使用的多坐标系描述方法以及DFUAV的姿态表示方法。在无人机的建模分析中，为简化模型复杂度便于控制算法的设计，有必要假设无人机是刚体。然后采用在刚体上应用广泛的牛顿-欧拉方法推导出DFUAV的刚体运动学模型和动力学模型，得到飞行控制的刚体模型。最后分析了作用在涵道上的力与力矩并作出总结。
...
自主搭建的DFUAV试验样机的实物及结构如图\ref{DFUAV}所示。DFUAV的主要结构包括机翼、机身、尾翼、涵道风扇、电机、电调、电池、飞控等部分。其中，机翼是DFUAV的承载结构，机身是DFUAV的主要载荷结构，尾翼是DFUAV的稳定结构，涵道风扇是DFUAV的动力来源，电机是涵道风扇的驱动部件，电调是电机的控制部件，电池是DFUAV的能量来源，飞控是DFUAV的智能控制部件。
% \begin{figure}[htbp]
% 	\centering
...
% 		\caption{\label{DFUAV1}本文研究所使用的DFUAV}
% 	\end{minipage}%
% \end{figure}
\begin{figure}[htbp]
	\centering
...
涵道底部的控制舵面是DFUAV的主要控制机构，在飞行过程中控制舵面的运动会产生气动力和力矩，这些力矩是造成DFUAV进行滚转、俯仰、偏航姿态变化的主要原因。如图所示，本文研究的DFUAV配置有四片交叉排列的控制舵面，定义机体系${\boldsymbol{O}_b}-{\boldsymbol{X}_b}$轴正方向下方位置对应的控制舵面为1号舵，从上往下按照顺时针方向依次编号为2、3、4号舵。1、3号舵的同向运动用于产生滚转力矩，2、4号舵的同向运动用于产生俯仰力矩，4个舵的共同差动用于产生偏航力矩。定义控制舵面的偏转角度为$\delta_i$，其中$i=1,2,3,4$分别对应4个控制舵面，向右偏转为正方向，零位点保持与机体系${\boldsymbol{O}_b}-{\boldsymbol{Z}_b}$轴平行。
...
因此，由控制舵面产生的合力与合力矩可以表示为\cite{pflimlinModelingAttitudeControl2010a,manzoorCompositeObserverbasedRobust2023}：
    ...
    \label{eq_36}
    \end{gather}
式\eqref{eq_36}中的$l_1$和$l_2$分别为涵道的力臂，如图所示。事实上，控制舵面的偏转角仅在DFUAV的失速迎角的范围内才有效果\cite{9787121348440000}，超出该范围后，姿态将难以控制。因此需要对偏转角度进行限幅处理：
\begin{equation}
...
固定气动面是指位于涵道内部，涵道风扇下方的固定装置。其特殊的外形结构用于在涵道风扇滑流中产生反扭矩来抵消DFUAV单一风扇产生的风扇扭矩$\boldsymbol{M}_{fan}^b$，固定气动面的反扭距效应可以近似表示为\cite{1020333010.nh}：
\begin{equation}
...
经过恰当的设计，固定气动面产生的反扭距$\boldsymbol{M}_{flap}^b$可以抵消大部分的风扇扭矩$\boldsymbol{M}_{gyro}^b$。在实践过程中很难达到理想情况，即$\boldsymbol{M}_{flap}^b + \boldsymbol{M}_{gyro}^b = \boldsymbol{0}$，所以剩余的力矩部分将由控制舵面的固定偏置产生偏航力矩来进行补偿。
